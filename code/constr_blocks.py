...
def show_text_blocks_in_row(image, rows):
    ...
    for row in rows:
        ...
        # show_image(image_to_show)
    # save_image(image_to_show, "images/text_comp_rows_{}.jpg".format(image_name))
    # save_image(image_to_show, "images/text_blocks/text_blocks_{0}-test.jpg".format(image_name))
...
# Load text elem
with open('data/data_rows_ccomp_{0}.data'.format(image_name), 'rb') as filehandle:
...
with open('data/distance_data.csv', 'r') as csvFile:
...
def save_text_blocks():
    with open("data/data_text_blocks_{0}.data".format(image_name), 'wb') as outfile:
...
def save_cropped_blocks(image, text_blocks_rows):
...
# save_cropped_blocks(binary_image, text_blocks_rows)

# filename_for_recognized = "images/cropped_blocks/{}.txt".format(image_name)
# recognized = open(filename_for_recognized, 'w+')

# with PyTessBaseAPI(path='C:/pr/conn_comp/tessdata', lang='rus+eng', psm=PSM.SINGLE_BLOCK) as api:
#     for ind_row, blocks_row in enumerate(text_blocks_rows):
#         row = ''
#         for ind_block, text_block in enumerate(blocks_row):
#             api.SetImageFile("images/cropped_blocks/{0}/{1}-{2}.jpg".format(image_name, ind_row, ind_block))
